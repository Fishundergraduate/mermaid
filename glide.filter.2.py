...
df1 = pd.read_csv(dataDir+"./present/filtered.csv",header=None, names=["smi","d","q"])
#df1 = pd.read_csv(dataDir+"./present/aizy.pass.csv", names=["smi","d","q"])
#df1 = pd.read_csv(dataDir+"./present/aizy.pass.csv", names=["smi","d"],sep=" ")
...
for i, ent in tqdm(df1.iterrows()):
    ...
    if m < -4:
        ...
        qed = QED.qed(mol)
        with open(dataDir+"./present/glide.passed.csv","a") as f2:
        ...
        #subprocess.run(f"sleep 60")
        #subprocess.run(f"mv {GLIDE_PIPELINE}/{index}/out.png {dataDir}./present/dock_{ent.smi}.png")
        #break
    #elif m > -4:
